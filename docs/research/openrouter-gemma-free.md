...
> **Runtime update — 2026-08-26:** This document preserves the earlier Gemma-specific assessment. The active runtime now uses the ordered pool documented in the [README](../../README.md), sourced from OpenRouter's [free text-model intelligence ranking](https://openrouter.ai/models?variant=free&output_modalities=text&order=intelligence-high-to-low) and filtered for this chat's JSON/context requirements. The current order is GLM 5.2, MiniMax M3, MiniMax M2.7, Gemma 4 31B, Gemma 4 26B, Nemotron 3 Super, and Dots3 Note. Model-specific cooldowns and the zero-cost checks remain authoritative.
...
OpenRouter says an account that has **purchased at least $10 in credits all-time** receives **1,000 free-model requests per day**, up from 50; the limit is shared by free-model requests rather than granted per model. OpenRouter's first-party rate-limit guidance also gives **20 requests per minute**, unchanged after the purchase, and says failed attempts count toward the daily allowance. Extra API keys or accounts do not raise these platform limits. Thus a user question that tries two OpenRouter models can consume two requests before Gemini/Groq fallback. The wording is “purchased”; a displayed balance that did not come from a qualifying purchase is not enough evidence of the higher tier. [OpenRouter limits](https://openrouter.ai/docs/api_reference/limits.md), [OpenRouter FAQ](https://openrouter.ai/docs/faq), [OpenRouter free-inference guidance](https://openrouter.ai/blog/tutorials/how-to-get-the-lowest-cost-llm-inference-on-openrouter/)
...
3. Keep OpenRouter provider fallbacks disabled. Use one reviewed exact-free ID per request, then fall back directly to Gemini and Groq; this avoids spending multiple counted free attempts when shared pools are unavailable.
